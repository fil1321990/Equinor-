...
import { formatTimeRemaining, getOffsetMs } from "./utils/time";
...
const ProductPromoClosingTag = ({ targetDate }: { targetDate: string }) => {
  ...
  return (
    <div className="flex items-center gap-1.5">
      {timeLeft.years > 0 && <div className="bg-black text-white rounded-[8px] px-3 h-[32px] flex items-center justify-center font-black text-[13px] min-w-[42px] shadow-sm">{timeLeft.years} Yr</div>}
      {(timeLeft.years > 0 || timeLeft.days > 0) && <div className="bg-black text-white rounded-[8px] px-3 h-[32px] flex items-center justify-center font-black text-[13px] min-w-[42px] shadow-sm">{timeLeft.days} Day</div>}
      <div className="bg-black text-white rounded-[8px] w-[36px] h-[32px] flex items-center justify-center font-black text-[14px] shadow-sm">
        {timeLeft.hours.toString().padStart(2, '0')}
      </div>
      <span className="text-black font-black text-lg leading-none -mt-0.5">:</span>
      <div className="bg-black text-white rounded-[8px] w-[36px] h-[32px] flex items-center justify-center font-black text-[14px] shadow-sm">
...
const ProductPromoEndTimer = ({ targetDate }: { targetDate: string }) => {
  const [timeLeft, setTimeLeft] = useState({ years: 0, days: 0, hours: 0, minutes: 0, seconds: 0 });
  ...
  return (
    <div className="flex items-center gap-1.5 scale-[1.15] origin-right">
      {timeLeft.years > 0 && <div className="bg-[#EF4444] text-white rounded-[8px] px-3 h-[32px] flex items-center justify-center font-black text-[13px] min-w-[42px] shadow-sm">{timeLeft.years} Yr</div>}
      {(timeLeft.years > 0 || timeLeft.days > 0) && <div className="bg-[#EF4444] text-white rounded-[8px] px-3 h-[32px] flex items-center justify-center font-black text-[13px] min-w-[42px] shadow-sm">{timeLeft.days} Day</div>}
      <div className="bg-[#EF4444] text-white rounded-[8px] w-[36px] h-[32px] flex items-center justify-center font-black text-[14px] shadow-sm">
        {timeLeft.hours.toString().padStart(2, '0')}
      </div>
      <span className="text-[#EF4444] font-black text-lg leading-none -mt-0.5">:</span>
      <div className="bg-[#EF4444] text-white rounded-[8px] w-[36px] h-[32px] flex items-center justify-center font-black text-[14px] shadow-sm">
        {timeLeft.minutes.toString().padStart(2, '0')}
      </div>
      <span className="text-[#EF4444] font-black text-lg leading-none -mt-0.5">:</span>
...
function MainApp() {
  ...
  const products = useMemo(() => {
    return rawProducts.map(p => {
      if (p.name.toLowerCase() === 'vip team exclusive project') {
        return { ...p, fixedDailyReturn: 300, days: 500 };
      }
      return p;
    });
  }, [rawProducts]);
  ...
  const [successAnimType, setSuccessAnimType] = useState<"deposit" | "withdraw" | "general">("general");
  useEffect(() => {
                  ...
                  {Array.from({ length: checkinCellCount }, (_, i) => {
                    ...
                    return (
                      <div key={day} className={`aspect-square border rounded-xl relative flex items-end justify-center pb-1.5 ${checked ? "bg-[#a88cff] border-white" : "bg-[#a88cff]/50 border-white/40"}`}>
                        ...
                        <span className="text-white font-bold text-[14px]">{day}</span>
                      </div>
                    );
                  })}
                </div>
              </div>
              <div className="px-4 mt-6 pb-20 w-full flex-shrink-0">
                <button 
                  onClick={handleCheckIn}
                  className="w-full bg-[#ffd24d] text-[#1a1e4e] font-bold text-[16px] py-3.5 rounded-full shadow-[0_4px_12px_rgba(255,210,77,0.3)] active:scale-[0.98] transition-transform"
                >
                  Sign in immediately
                </button>
              </div>
...
{activeTab === "product" && (
                          ...
                          <div className="px-4 pb-4">
                            <div className="bg-[#F8F9FA] rounded-[12px] py-2.5 px-3 grid grid-cols-2 gap-y-3 gap-x-1">
                              <div className="flex items-center whitespace-nowrap">
                                ...
                                <span className="text-[#1C1C1E] font-black text-[12px] ml-1">{totalIncome.toLocaleString()}</span>
                              </div>
                              <div className="flex items-center justify-end whitespace-nowrap">
                                <span className="text-[#6B7280] text-[11px]">Quota:</span>
                                <span className="text-[#1C1C1E] font-black text-[12px] ml-1">{plan.maxQuota || ''}</span>
                              </div>
                              <div className="flex items-center whitespace-nowrap">
                                <span className="text-[#6B7280] text-[11px]">Cycle:</span>
                                <span className="text-[#1C1C1E] font-black text-[12px] ml-1">{plan.days} Days</span>
                              </div>
                              <div className="flex items-center justify-end whitespace-nowrap">
                                <span className="text-[#6B7280] text-[11px]">Daily income:</span>
                                <span className="text-[#1C1C1E] font-black text-[12px] ml-1">{calculatedDailyReturn.toLocaleString()}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="p-2 space-y-2 max-h-[300px] overflow-y-auto scrollbar-hide">
                    {products.filter(p => p.type !== 'redemption_code').map(p => (
                      <div key={p.id} className="flex flex-col bg-white/5 p-3 rounded-xl border border-white/5 gap-2">
                        <div className="flex justify-between items-start">
                          <div>
                            <div className="text-white font-black text-[18px]">{p.name}</div>
                            <div className="flex flex-wrap gap-2 mt-1">
                              <span className="text-[10px] bg-[#00D4FF]/20 text-[#00D4FF] px-2 py-0.5 rounded uppercase font-black">{p.type}</span>
                              <span className="text-[10px] bg-white/10 text-white px-2 py-0.5 rounded font-black">{p.roi}% ROI</span>
                              <span className="text-[10px] bg-white/10 text-white px-2 py-0.5 rounded font-black">₦{p.min}</span>
                                ...
                                try {
                                  return (
                                    <div key={tx.id} className="p-3 border-b border-white/5 flex justify-between items-center text-sm hover:bg-white/5 transition-colors">
                                      ...
                                      <div className="text-right">
                                        <div className="font-bold text-white text-[15px]">{typeof formatCurrency === 'function' ? formatCurrency(tx.amount) : tx.amount}</div>
        ...
        {activeModal === "depositCheckout" && (
          <div className="absolute inset-0 z-50 flex flex-col bg-slate-50 overflow-y-auto pt-safe">
            ...
            {/* Step Indicator */}
            <div className="px-6 py-5 bg-white border-b border-slate-100 flex items-center justify-between relative shadow-sm z-10">
              <div className="absolute top-[28px] left-10 right-10 h-1 bg-slate-100 -translate-y-1/2 z-0 rounded-full"></div>
              <div className="absolute top-[28px] left-10 h-1 bg-blue-600 -translate-y-1/2 z-0 transition-all duration-500 rounded-full" style={{ width: depositCheckoutStep === 1 ? '0%' : depositCheckoutStep === 2 ? '50%' : '100%', maxWidth: 'calc(100% - 5rem)' }}></div>
              
              <div className="relative z-10 flex flex-col items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 ${depositCheckoutStep >= 1 ? 'bg-blue-600 text-white shadow-md shadow-blue-500/30 scale-110' : 'bg-slate-100 text-slate-400'}`}>1</div>
                <span className={`text-[10px] font-bold uppercase tracking-widest transition-colors duration-300 ${depositCheckoutStep >= 1 ? 'text-blue-600' : 'text-slate-400'}`}>Copy</span>
              </div>
              <div className="relative z-10 flex flex-col items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 ${depositCheckoutStep >= 2 ? 'bg-blue-600 text-white shadow-md shadow-blue-500/30 scale-110' : 'bg-slate-100 text-slate-400'}`}>2</div>
                <span className={`text-[10px] font-bold uppercase tracking-widest transition-colors duration-300 ${depositCheckoutStep >= 2 ? 'text-blue-600' : 'text-slate-400'}`}>Transfer</span>
              </div>
              <div className="relative z-10 flex flex-col items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 ${depositCheckoutStep >= 3 ? 'bg-blue-600 text-white shadow-md shadow-blue-500/30 scale-110' : 'bg-slate-100 text-slate-400'}`}>3</div>
                <span className={`text-[10px] font-bold uppercase tracking-widest transition-colors duration-300 ${depositCheckoutStep >= 3 ? 'text-blue-600' : 'text-slate-400'}`}>Confirm</span>
              </div>
            </div>
        ...
        {activeModal === "successAnimated" && (() => {
          let primaryColor = "#34C759";
          let gradientStart = "#4ade80";
          ...
          <div className="absolute inset-0 z-[999] bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
            <motion.div 
              ...
              className="bg-white border border-slate-200 rounded-3xl p-8 max-w-[340px] w-full flex flex-col items-center relative overflow-hidden"
              style={{ boxShadow: `0 20px 60px rgba(${rgbColor},0.3)` }}
            >
              <div className="absolute top-0 w-full h-2" style={{ background: `linear-gradient(to right, ${primaryColor}, ${primaryColor}80)` }} />
              {/* Animated Glow */}
              <motion.div 
                animate={{ 
                  background: [
                    `radial-gradient(circle, rgba(${rgbColor},0.15) 0%, rgba(255,255,255,0) 70%)`, 
                    `radial-gradient(circle, rgba(${rgbColor},0.1) 0%, rgba(255,255,255,0) 70%)`, 
                    `radial-gradient(circle, rgba(${rgbColor},0.15) 0%, rgba(255,255,255,0) 70%)`
                  ] 
                }}
              ...
              <div className="relative z-10 w-24 h-24 mb-5">
                <motion.svg viewBox="0 0 50 50" className="w-full h-full" style={{ filter: `drop-shadow(0 4px 12px rgba(${rgbColor}, 0.4))` }}>
                  <motion.circle 
                    cx="25" cy="25" r="23" 
                    fill="none" 
              ...
              {successAnimAmount !== null && (
                <motion.div
                  ...
                  className="px-4 py-2 rounded-xl border font-black text-xl mb-3 z-10"
                  style={{ backgroundColor: `${primaryColor}15`, color: gradientEnd, borderColor: `${primaryColor}30`, boxShadow: `0 2px 10px rgba(${rgbColor},0.1)` }}
                >
              ...
              <motion.button
                ...
                transition={{ delay: 1 }}
                onClick={() => setActiveModal(null)}
                className="w-full py-4 rounded-2xl text-white font-black uppercase tracking-wider active:scale-95 transition-transform z-10 relative shadow-lg text-[15px]"
                style={{ background: `linear-gradient(to right, ${gradientStart}, ${gradientEnd})`, boxShadow: `0 10px 25px -5px rgba(${rgbColor},0.4)` }}
              >
                Continue
              </motion.button>
            </motion.div>
...
class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    ...
    // Log error details to a remote monitoring service or an internal log table
    try {
      (async () => {
        ...
        if (insertErr) console.error("Failed to log error:", insertErr);
      })();
    } catch (e) {
                  ...
                  <div className="flex-1 overflow-y-auto w-full relative pb-[140px] flex flex-col custom-scrollbar">
                    ...
                    {isLoadingOrders ? (
                      ...
                      filteredInvestments.map(inv => {
                        ...
                        return (
                          <div key={inv.id} className="relative bg-[#F8F9FF] rounded-[20px] mb-6 shadow-md overflow-hidden flex flex-col p-4">
                            
                            <div className="flex justify-between items-center mb-3">
                              <span className="bg-[#FFE5E5] text-[#FF4444] px-2 py-1 rounded-[6px] font-bold text-[12px]">
                                T+{inv.tPlusDays || 1}
                              </span>
                              <div className="flex items-center gap-1.5">
                                <span className="text-[12px] font-bold text-gray-700 uppercase">CP{new Date(inv.startDate).getTime()}{inv.id.substring(0, 4)}</span>
                                <div className="opacity-40">
                                   <Barcode className="w-8 h-4 text-gray-700" strokeWidth={1} />
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="flex justify-between items-start mb-4">
                              <div className="flex flex-col flex-1 min-w-0 pr-2">
                                <h3 className="font-bold text-[#1E293B] text-[18px] leading-tight mb-2 truncate">{inv.planName}</h3>
                                <div className="bg-[#E8E9FF] rounded-xl p-3 text-center text-[#5B5FEF] font-black text-[18px] leading-none">
                                  ₦{inv.amount.toLocaleString()}
                                </div>
                              </div>
                            </div>
                            
                            <div className="bg-[#FAFBFF] rounded-[16px] p-4 flex flex-col gap-4">
                               <div className="flex justify-between items-center">
                                 <span className="text-[#666666] text-[14px]">Profit:</span>
                                 <span className="text-[#FF4444] font-bold text-[18px] ml-2 leading-none">+{profitAccrued.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})} ₦</span>
                               </div>
                               
                               <div className="grid grid-cols-3 gap-3">
                                 <div className="bg-[#E8E9FF] rounded-xl p-3 flex flex-col items-center justify-center text-center">
                                    <span className="text-[#5B5FEF] font-black text-[14px] leading-none mb-1">₦{dailyIncome.toLocaleString()}</span>
                                    <span className="text-[#5B5FEF] opacity-80 text-[10px] uppercase font-bold tracking-wider">Daily</span>
                                 </div>
                                 <div className="bg-[#E8E9FF] rounded-xl p-3 flex flex-col items-center justify-center text-center">
                                    <span className="text-[#5B5FEF] font-black text-[14px] leading-none mb-1">{invCycleLength}</span>
                                    <span className="text-[#5B5FEF] opacity-80 text-[10px] uppercase font-bold tracking-wider">Days</span>
                                 </div>
                                 <div className="bg-[#E8E9FF] rounded-xl p-3 flex flex-col items-center justify-center text-center">
                                    <span className="text-[#5B5FEF] font-black text-[14px] leading-none mb-1">₦{(dailyIncome * invCycleLength).toLocaleString()}</span>
                                    <span className="text-[#5B5FEF] opacity-80 text-[10px] uppercase font-bold tracking-wider">Total</span>
                                 </div>
                               </div>
                               ...
                               {canCollect ? (
                                <button
                                  ...
                                  className={`bg-[#5B5FEF] text-white w-full h-[48px] rounded-[14px] font-black text-[15px] shadow-md transform transition flex flex-col justify-center items-center ${collectingIds[inv.id] ? 'opacity-80 scale-95' : 'active:scale-[0.98]'}`}
                                >
                                  {collectingIds[inv.id] ? "..." : "Get"}
                                </button>
                              ) : isExpired ? (
                                <span className="w-full flex items-center justify-center text-[#34C759] font-black px-5 py-3 border border-[#34C759]/20 rounded-[14px] bg-[#34C759]/5 text-[15px]">
                                  {inv.status === 'completed' ? 'Completed' : 'Expired'}
                                </span>
                              ) : (
                                <button
                                  disabled
                                  className="w-full bg-[#E8E9FF] text-[#A0A0A0] h-[48px] rounded-[14px] font-black text-[15px] cursor-not-allowed flex items-center justify-center gap-2"
                                >
                                  <div className="flex gap-1">
                                    <span className="bg-[#1A1A1A] text-white px-2 py-1 rounded-lg text-sm">{String(hoursLeft).padStart(2, '0')}</span>
                                    <span className="text-[#1A1A1A] font-bold self-center">:</span>
                                    <span className="bg-[#1A1A1A] text-white px-2 py-1 rounded-lg text-sm">{String(minutesLeft).padStart(2, '0')}</span>
                                    <span className="text-[#1A1A1A] font-bold self-center">:</span>
                                    <span className="bg-[#1A1A1A] text-white px-2 py-1 rounded-lg text-sm">{String(secondsLeft).padStart(2, '0')}</span>
                                  </div>
                                </button>
                              )}
                              
                              <div className="w-full bg-[#E2E8F0] h-[6px] rounded-full overflow-hidden mt-1">
                                <div 
                                   className="h-full bg-gradient-to-r from-[#5B3DF6] to-[#7B2FF7] transition-all duration-1000"
                                  style={{ width: `${Math.min(100, (readingElapsedMs / maxElapsedCycleMs) * 100)}%` }}
                                />
                        ...
                        return (
                          <div key={inv.id} className="relative bg-[#F4F6FC] rounded-[20px] mb-6 shadow-md overflow-hidden flex flex-col p-4">
                            {/* Image Header */}
                            <div className="w-full relative mb-4">
                              <div className={`relative w-full aspect-[16/9] rounded-[20px] ${inv.planName.toLowerCase() === 'vip team exclusive project' ? 'bg-gradient-to-r from-red-900 to-black' : 'bg-gradient-to-br from-[#1F2937] to-[#111827]'} flex items-center justify-center overflow-hidden`}>
                                {product?.imageUrl ? (
