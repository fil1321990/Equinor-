...
// The original signature and body of globalMutate was duplicated.
// Let's strip the globalMutate block out and replace it.

const regex = /const globalMutate = async \(args\?: any\) => \{[\s\S]*?\}\s*};\s*useEffect\(\(\) => \{/m;
