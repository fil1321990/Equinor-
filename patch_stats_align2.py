...
target2 = '''                              <div className="flex items-center font-bold text-[#1A1A1A] text-[12px] whitespace-nowrap">
                                {(() => {
...
replacement2 = '''                              <div className="flex items-center justify-end font-bold text-[#1A1A1A] text-[12px] whitespace-nowrap">
                                {(() => {
                                  const userBoughtCount = investments.filter(inv => inv.userId === currentUser?.id && inv.planName === plan.name).reduce((sum, inv) => sum + (inv.quantity || 1), 0);
                                  return (
                                    <>
